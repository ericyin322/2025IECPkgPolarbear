...
export class GameEngine {
  ...
  setStatus(txt: any) { this.statusEl.textContent = txt; }
  setScore(n: any) { this.scoreEl.textContent = String(n); }
  showHint(show: any) { this.hintEl.classList.toggle('show', !!show); }
    ...
    switch (this.Game.state) {
      case 'walking': {
        ...
        if (this.Game.timer >= this.Game.walkDuration) {
          ...
          this.Game.state = 'encounter'; this.Game.timer = 0; this.setStatus('靠近目標…');
        }
        break;
      }
      case 'encounter': {
        // walk closer until within range
        const targetX = this.blizzard.currentSeal.group.position.x - 2.5;
        const dx = Math.min(this.blizzard.bear.state.speed * dt, Math.max(0, targetX - this.blizzard.bear.group.position.x));
        this.blizzard.bear.group.position.x += dx;
        this.blizzard.camera.position.x += dx * 0.6; this.blizzard.controls.target.x += dx * 0.6;
